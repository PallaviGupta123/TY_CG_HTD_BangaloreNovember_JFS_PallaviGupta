...
public class ContractLogin {
	...
	public static void main(int contractorId) {
		@SuppressWarnings("resource")
		Scanner sc=new Scanner(System.in);
		IContractServices service=new ContractServices();
		IHauliersService haulierService= Factory.objectHaulierService();

		IRequestService requestService= Factory.objectRequestService();
		...
		while(!logout) {
			...
			case 1:
				...
				// Get all haulier
				List<Hauliers> listOfHauliers=haulierService.getAllHauliers();
				System.out.println(listOfHauliers.toString());
				System.out.println("Enter Haulier Id from above list: ");
				String haulerId=null;
				boolean isHaulierSelected=false;
				boolean isValidHaulier=false;
				...
				do {
					try {
						...
						{
							for (Hauliers hauliers : listOfHauliers) 
							{
								if(hauliers.getHaulierId()==Integer.parseInt(haulerId))
									
								{
							contract.setHaulierId(Integer.parseInt(haulerId));
							isValidHaulier=true;
						} 
							
								else {

							throw new ValidationException("Please enter digit only!");
						}
							}
						}
					}
				...
				} while (!Validation.isNumber(haulerId) ||  !isValidHaulier);	

				List<Request> listofRequests=requestService.getAllRequest(0);
				System.out.println(listofRequests.toString()); 

				// get all request
				System.out.println("Enter Request Id from above list: ");
